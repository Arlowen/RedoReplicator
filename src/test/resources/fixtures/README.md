...
Small properties fixtures under `redo-header/`, `value-types/` and `multi-row/`
are generated by the corresponding fixed-baseline C++ probes under
`scripts/baseline/` and assert byte-level parity where a complete redo file is
not required.
