...
probe_binary="$project_dir/target/compressed-row-probe"
...
c++ -std=c++17 -DCTXASSERT=0 \
    ...
    "$project_dir/scripts/baseline/compressed-row-probe.cpp" \
    "${object_files[@]}" \
    -pthread \
    -o "$probe_binary"

"$probe_binary" > "$output_file"
echo "Compressed-row parity output: $output_file"
